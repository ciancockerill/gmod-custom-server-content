...
SWEP.PrintName 				= "[DG] Tactical Tablet"
SWEP.Author 				= "InGame"
SWEP.Spawnable 				= true
...
SWEP.ViewModelBoneMods = {
	...
	["ValveBiped.Bip01_R_Forearm"] = { scale = Vector(1, 1, 1), pos = Vector(0, -2, -4.5), angle = Angle(0, 0, 0) },
...
SWEP.VElements = {
	["tablet"] = { type = "Model", model = "models/nirrti/tablet/tablet_sfm.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(6, 7.6, -8), angle = Angle(30, 97.5, -115), size = Vector(1.5, 1.5, 1.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 1, bodygroup = {} }
}
...
if CLIENT then
	function SWEP:PostDrawViewModel(vm, wep, ply)
		if IsValid(vm) then
			...
			baseAng:RotateAroundAxis(baseAng:Right(), 30)
...
function DGInGame.GivePredatorWeapon(ply)
	if not IsValid(ply) or not ply:IsPlayer() then return end

	ply:ScreenFade(1, Color(0,0,0), 2, 1.5)

	net.Start("tablet_close")
	net.Send(ply)

	ply:Give("predator_m")
	ply:SelectWeapon("predator_m")

end

function DGInGame.GiveAC130(ply)
	if not IsValid(ply) or not ply:IsPlayer() then return end
	...
	ply:Give("ac130_gns")
	ply:SelectWeapon("ac130_gns")

end

if SERVER then

    util.AddNetworkString("tablet_open")
    util.AddNetworkString("tablet_close")
	util.AddNetworkString("GivePredatorWeapon")
	util.AddNetworkString("GiveAC130")

	net.Receive("GivePredatorWeapon", function(_, ply)
		DGInGame.GivePredatorWeapon(ply)
	end)
	net.Receive("GiveAC130", function(_, ply)
		DGInGame.GiveAC130(ply)
	end)
end
